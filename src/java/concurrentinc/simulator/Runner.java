...
public class Runner
  ...
  public static void run( PrintWriter writer ) throws ExecutionException, InterruptedException, SimulationException
    ...
    for( int inputSizeMb = 1; inputSizeMb < 1 * 1024 * 1024; inputSizeMb += 100 )
      {
      for( int shuffleSizeMb = 1; shuffleSizeMb < 1 * 1024 * 1024; shuffleSizeMb += 100 )
        {
        for( int outputSizeMb = 1; outputSizeMb < 1 * 1024 * 1024; outputSizeMb += 100 )
          {
          for( int numMappers = 1; numMappers < 1000; numMappers++ )
            {
            for( int numReducers = 1; numReducers < 1000; numReducers++ )
              {
